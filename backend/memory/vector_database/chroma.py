...
class Chroma:
    ...
    def from_chunks(self, chunks: list[Document]) -> list[str]:
        ...
        """
        # `clean` defaults to `to_ascii=True` and `lower=True`. Those defaults transliterate non-ASCII
        # text (Chinese, for instance, is romanised into pinyin) and lowercase the result, silently
        # rewriting the content before it is embedded. The query path (`embed_query`) applies no such
        # transformation, so index and query would stop sharing a vector space. Keep the original text.
        texts = [clean(doc.page_content, no_emoji=True, to_ascii=False, lower=False) for doc in chunks]
