...
async def list_documents(session: SessionDep):
    """
    List every document currently held in the knowledge base.

    Reads from the persistent registry rather than from process memory, so the listing also covers
    documents ingested by `scripts/memory_builder.py` and survives a backend restart.

    Returns:
        DocumentListResponse containing a list of all document metadata.
    """
    records = DocumentRegistry(session).get_all()
